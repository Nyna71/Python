...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    holdoutSetErrors = []
    for kval in kvals:
        holdoutPoints, trainingPoints = generateTrainingSet(points, len(points)*20/100)
        
        trainingClusters, maxSmallest = kmeans(trainingPoints, kval, cutoff, County)
        holdoutClusters, maxSmallest = kmeans(holdoutPoints, kval, cutoff, County)
        
        trainingSetErrors.append(getClusterError(trainingClusters))
        holdoutSetErrors.append(getClusterError(holdoutClusters))
    
    pylab.figure()
    pylab.plot(kvals, trainingSetErrors)
    pylab.plot(kvals, holdoutSetErrors)
    
    errorRatio = []
    for i in range(len(trainingSetErrors)):
        errorRatio.append(holdoutSetErrors[i] / trainingSetErrors[i])
...
def getClusterError(clusters):
    ...
    for cluster in clusters:
        totalDistance += getClusterDistance(cluster)
    return totalDistance**2
        
def getClusterDistance(cluster):
    """ returns the total distance of all points to the cluster's centroid point
    """
    distance = 0
...
def generateTrainingSet (points, p):
    holdoutPoints = random.sample(points, p)
    trainingPoints = []
    
    for point in points:
        if point not in holdoutPoints:
            trainingPoints.append(point)
    return holdoutPoints, trainingPoints
    
def getMyCounty(points, countyName):
    for i in range(3):
        clusters, maxSmallest = kmeans(points, 50, 0.1, County)
        for cluster in clusters:
            if cluster.contains(countyName):
                print cluster
...
def graphPredictionErr(points, dimension, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    for kval in kvals:
        ...
        #Update the weights for each point in the training set before applying k-means clustering
        for point in trainingPoints:
            point.updateWeights(dimension)
        
        trainingClusters, maxSmallest = kmeans(trainingPoints, kval, cutoff, County)
        
        povertyDiff = 0.0
        for point in holdoutPoints:
            cluster = findClosestCluster(point, trainingClusters)
            avgPoverty = getAveragePoverty(cluster)
            attrs = point.getOriginalAttrs()
            pointPoverty = attrs[2]
            povertyDiff += (pointPoverty - avgPoverty)**2
            
        povertiesDiff.append(povertyDiff)
        
    pylab.figure()
    pylab.plot(kvals, povertiesDiff)
...
def getAveragePoverty(cluster):
    ...
    for point in points:
        attrs = point.getOriginalAttrs()
        totalPoverty += attrs[2]
        
    return totalPoverty / len(points)
        
        
def findClosestCluster(point, clusters):
    smallestDistance = point.distance(clusters[0].getCentroid())
    clusterIndex = 0
    for i in range(len(clusters)):
        distance = point.distance(clusters[i].getCentroid())
        if distance < smallestDistance:
            smallestDistance = distance
            clusterIndex = i
