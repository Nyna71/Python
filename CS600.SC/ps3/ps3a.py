...
def get_word_score(word, n):
    ...
    #Sum up value from each letter
    for letter in word:
        wordScore += SCRABBLE_LETTER_VALUES.get(letter)
    
    wordScore *= len(word)
    
    #Add eventually 50 bonus points if all letters are used
    if(len(word) == n):
        wordScore += 50
...
def is_valid_word(word, hand, word_list):
    ...
    #print 'Word', word, 'is in word list'
...
def play_game(word_list):
    ...
    * If the user inputs anything else, ask them again.
    ...
    hand={}
    while(True):
        userSelection = raw_input('Enter n(ew), r(eplay) or e(xit): ')
        if(userSelection == 'e'):
            break
        if(userSelection == 'n'):
            hand = deal_hand(HAND_SIZE)
            play_hand(hand, word_list)
        if(userSelection == 'r' and calculate_handlen(hand) > 0):
            play_hand(hand, word_list)
